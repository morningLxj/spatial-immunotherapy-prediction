...
\textbf{C1Q/SPP1 Macrophage Spatial Context as a Traceability Feature in Non-Small Cell Lung Cancer: A Public Multi-Cohort Cancer-Informatics Study}}
\author{\begin{minipage}{0.94\textwidth}\centering
Xiongjie Li\textsuperscript{1,*}, Fengyue Zhang\textsuperscript{2}, Chaojia Luo\textsuperscript{1}, and Xuan Xu\textsuperscript{1}\\[0.5em]
\small \textsuperscript{1}Department of Cardiothoracic, Thyroid and Breast Surgery, Qianjiang Jianghan Oilfield General Hospital, No. T32, Guanghua Ankang Road, Qianjiang 433100, Hubei, China\\
\small \textsuperscript{2}Department of Gastroenterology, Qianjiang Jianghan Oilfield General Hospital, Qianjiang 433100, Hubei, China\\[0.5em]
\small *Correspondence: Xiongjie Li, MD; Department of Cardiothoracic, Thyroid and Breast Surgery, Qianjiang Jianghan Oilfield General Hospital, Qianjiang 433100, Hubei, China\\
\small Email: \href{mailto:surobert518@163.com}{surobert518@163.com}; ORCID: \href{https://orcid.org/0009-0005-9507-6829}{0009-0005-9507-6829}
\end{minipage}}
\date{}
...
\begin{figure}[H]
  ...
  \caption{\textbf{C1Q-axis scoring, dimensionality, and cross-platform spatial-context checks.} (A) Distribution of the C1Q-axis score. (B) Elbow-plot summary used to inspect dimensional structure. (C) Silhouette analysis used to assess cluster separation. (D) Visium-based technical-control comparison. (E) CosMx-based biological-signal comparison. These panels are interpreted as quality and traceability checks rather than patient-level confirmation.}
  \label{fig:S1}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{\textbf{Exploratory correlation, survival, and pathway context for immune-related features.} The panels summarize gene-correlation structure, survival-associated stratification, and pathway-level enrichment for the candidate feature context. These analyses are descriptive and should not be interpreted as immune-checkpoint confirmation.}
  \label{fig:S4}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{\textbf{Exploratory expression differences for selected cancer-associated genes in GSE31210 score groups.} Boxplots show expression levels of selected genes, including \textit{PTEN}, \textit{TP63}, \textit{TP53}, \textit{BRAF}, and \textit{NOTCH1}, across high- and low-score groups in the GSE31210 traceability cohort. These panels provide exploratory context only and are not used as the primary basis for candidate prioritization. Statistical significance was assessed using the Mann-Whitney U test (* $P < 0.050$, ** $P < 0.010$, *** $P < 0.001$, **** $P < 0.0001$).}
  \label{fig:S7}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{scoring_sensitivity/FigureY_hotspot_collapse.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{scoring_sensitivity/FigureS12_mechanism_modules.pdf}
...
\noindent\textbf{Supplementary table index.}
Table S1, retrospective score behavior on training, internal cross-validation, test, and external sets;
Table S2, functional enrichment terms associated with the 91-feature set;
Table S3, eQTL-driven regulatory-network edge list;
Table S4, stable features identified by nested cross-validation;
Table S5, spatial autocorrelation and hotspot analysis;
Table S6, hub-gene and immune-context correlations;
Table S7, Mendelian-randomization analysis results;
Table S8, TME-subtype differential-expression context;
Table S9, external-cohort covariate summaries;
Table S10, cell-type-specific eQTL reporting boundary and interpretation;
Table S11, MR diagnostic parameters for prioritized genes;
Table S12, spatial transcriptomics sample information; and
Table S13, multivariable Cox regression analysis of the retrospective score.
...
\begin{landscape}
  ...
  \newgeometry{left=1cm, right=1cm, top=2cm, bottom=2cm}
  {
    ...
    \begin{longtable}{l >{\raggedright\arraybackslash}p{10cm} *{5}{c}}
      \caption{\textbf{Apparent training-set metrics for audit transparency only}} \label{tab:Table_S1a}                                                                                               \\
      \toprule
    ...
    \begin{longtable}{l *{5}{c} *{5}{c} c}
      \caption{\textbf{Retrospective score-behavior metrics in internal cross-validation, test, and external traceability sets}} \label{tab:Table_S1b}                                                                        \\
      \toprule
                          & \multicolumn{5}{c}{\textbf{Internal CV Set}} & \multicolumn{5}{c}{\textbf{Test Set}} & \textbf{Ext.}                                                                                    \\
      ...
      AdaBoost            & 0.657$\pm$0.04                              & 0.580$\pm$0.05                        & 0.550$\pm$0.10 & 0.558$\pm$0.06 & 0.657$\pm$0.05 & 0.657 & 0.588 & 0.476 & 0.526 & 0.743 & 0.650 \\
      Soft Voting         & 0.695$\pm$0.03                              & 0.642$\pm$0.04                        & 0.582$\pm$0.05 & 0.611$\pm$0.04 & 0.757$\pm$0.02 & 0.738 & 0.715 & 0.761 & 0.732 & 0.757 & -     \\
    ...
    \vspace{1em}
    \noindent\footnotesize\textbf{Note:} Summary of retrospective score-behavior metrics for candidate machine-learning algorithms.
    ...
    \textbf{Statistical Presentation:} Values in the \textit{Internal CV Set} column are expressed as mean $\pm$ standard deviation derived from 5-fold nested cross-validation, reflecting internal score stability rather than formal clinical evidence. Apparent training-set metrics in Table S1a are retained only for audit transparency and are not interpreted as evidence of generalizable performance.
    \textbf{Cohorts:} \textit{Training} and \textit{Test} sets are derived from the TCGA NSCLC cohort after feature availability and quality-control filtering; analysis denominators vary by endpoint and are documented in the source-data manifest. \textit{Ext.} denotes the GSE31210 traceability cohort with complete follow-up (N=178). The external AUC is reported only for the reference AdaBoost score used in the manuscript and is not used for algorithm selection.
...
\begin{longtable}{>{\raggedright\arraybackslash}p{8cm} c c c}
  \caption{\textbf{Functional enrichment terms associated with the 91-feature set.}} \label{tab:S2} \\
  \toprule
...
\vspace{1em}
\noindent\small \textbf{Note:} This table summarizes functional enrichment terms associated with the 91 feature set prioritized by nested cross-validation stability.
...
\textbf{Significance:} These enrichment terms provide biological context for the retrospective AdaBoost reference score and are not interpreted as pathway-level confirmation.
\newpage
...
\vspace{1em}
\noindent\small \textbf{Note:} Stable features were identified using nested 5-fold cross-validation repeated 5 times. Selection frequency indicates the number of times each feature was selected across all outer folds (maximum = 5). Only features selected in at least 3 folds are shown. The complete 91-feature set selected with 100\% frequency across all 5 outer folds is available in the supplementary data file: \texttt{nested\_cv\_selected\_features.csv}.
\newpage
\begin{longtable}{ll}
...
{\small
  ...
  \begin{longtable}{>{\raggedright\arraybackslash}p{0.18\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.16\textwidth}}
    \caption{\textbf{TME-subtype differential-expression context}} \label{tab:Table_S8} \\
    \toprule
...
\vspace{1em}
\noindent\small \textbf{Note:} Differential expression analysis across tumor microenvironment (TME) subtypes. FC: Fold Change; FDR: False Discovery Rate. Only top genes per subtype are shown; complete results are available in supplementary data files. This table provides subtype context and is not a colocalization or causal analysis.

% Table S9
\newpage
\begin{longtable}{lcccc}
    \caption{\textbf{External-cohort covariate summaries}} \label{tab:Table_S9} \\
  \toprule
...
\vspace{1em}
\noindent\small \textbf{Note:} External-cohort covariate summaries for survival endpoints. HR: Hazard Ratio; CI: Confidence Interval; OS: Overall Survival; RFS: Recurrence-Free Survival. Main external traceability estimates are interpreted descriptively rather than as confirmatory evidence.
...
{\small
  \begin{table}[H]
    \caption{\textbf{Cell-type-specific eQTL reporting boundary and interpretation}} \label{tab:Table_S10}
    \setlength{\tabcolsep}{4pt}
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.17\linewidth}>{\raggedright\arraybackslash}p{0.16\linewidth}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X}
      \toprule
      Analysis component & Reporting unit & Use in manuscript & Interpretation boundary & Reviewer note \\
      \midrule
      Cell-type eQTL context & Gene-level summary & Supports candidate prioritization & Not single-cell causal proof & SNP-level rows are not used as formal evidence in this submission \\
      MR sensitivity diagnostics & Gene-level MR diagnostics & Reported in Table S11 and Figures S9--S10 & Not mechanism or target validation & Interpreted only under instrumental-variable assumptions \\
      Spatial-context comparison & Section/sample contrast & Cross-checks candidate localization context & Not patient-level confirmation & Spots/cells are measurement units, not independent biological replicates \\
      \bottomrule
    \end{tabularx}
  \end{table}
}
...
\vspace{1em}
\noindent\small \textit{Note:} This table replaces SNP-level example rows with the reporting boundary used for cell-type eQTL context. Formal MR diagnostics are reported at gene level in Table S11 and Supplementary Figures S9--S10; no placeholder or illustrative SNP identifiers are included in the submission.
...
{\tiny
  ...
  \begin{longtable}{lcccccccc}
    \caption{\textbf{MR diagnostic parameters for prioritized genes}} \label{tab:Table_S11}                                 \\
    \toprule
...
{\small
  \begin{table}[H]
    \caption{\textbf{Spatial transcriptomics reporting summary and interpretation boundaries}} \label{tab:Table_S12}
    \setlength{\tabcolsep}{4pt}
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.16\linewidth}>{\raggedright\arraybackslash}p{0.18\linewidth}>{\centering\arraybackslash}p{0.11\linewidth}>{\centering\arraybackslash}p{0.11\linewidth}>{\raggedright\arraybackslash}p{0.20\linewidth}>{\raggedright\arraybackslash}X}
      \toprule
      Platform & Reporting unit & Independent patients & Tissue sections & Measurement scale & Interpretation boundary \\
      \midrule
      10x Visium spatial transcriptomics & Section-level spatial context & 1 & 8 & 55 $\mu$m spots; tissue-level transcriptomic mapping & Used for within-patient, section-level spatial-context mapping. Spot-level summaries are descriptive measurement units and are not independent biological replicates. \\
      NanoString CosMx SMI & Patient/section contrast and single-cell-resolution context & 5 & 8 & Approximately 800,000 profiled cells; 960-gene RNA panel & Used for cross-platform spatial-context inspection. Cell counts increase measurement resolution but do not increase the number of independent patients. \\
      \bottomrule
    \end{tabularx}
  \end{table}
}
...
\vspace{1em}
\noindent\small \textbf{Note:} This table reports cohort-level spatial-data structure rather than individual source accession identifiers. Source pages, processing notes, and claim-to-source mapping are provided in the source-data manifest and public repository. Spatial analyses are interpreted at the patient, section, or sample-contrast level; spots and cells are measurement units within those samples.

% Table S13
\newpage
\begin{table}[H]
  \caption{\textbf{Multivariable Cox regression analysis of the retrospective score}} \label{tab:Table_S13}
  \small
  \setlength{\tabcolsep}{4pt}
...
\textbf{Interpretation:} The C1Q signature remained associated with outcome in the training cohort and showed a compatible trend in the external cohort. Harmonized PD-L1/TMB comparator modeling was not available across the analyzed public cohorts and is not claimed as a completed baseline-comparison analysis.
\end{document}
